...
# Create guide labels
guide_label_1 = Label(root, text="FNリスト : u(t) - r(t) - sin(t, omega) - cos(t, omega) - delta(t) | s(t) - exp_decay(t, alpha) | e_1(t, alpha) - exp(t, alpha) | e(t, alpha)", anchor='e', justify=RIGHT)
guide_label_2 = Label(root, text="例 (れい) : u(t) - r(t-1) + 2*delta(t-2)", anchor='e', justify=LEFT)

# Position guide labels
guide_label_1.grid(row=0, column=0, columnspan=5, padx=10, pady=2, sticky=W)
guide_label_2.grid(row=1, column=0, columnspan=5, padx=10, pady=2, sticky=W)

# Create and position labels and entry fields for user inputs
t_start_label = Label(root, text="開始図 (かいしず) :")
t_start_label.grid(row=2, column=3)
t_start_entry = Entry(root, text=t_start, width=20, font=55)
t_start_entry.grid(row=2, column=4, pady=5, sticky=W)

t_end_label = Label(root, text="～で終了 (～でしゅうりょう) :")
t_end_label.grid(row=2, column=1)
t_end_entry = Entry(root, text=t_end, width=20, font=55)
t_end_entry.grid(row=2, column=2, pady=5, sticky=W)

problem_label = Label(root, text="問題 (もんだい) :")
problem_label.grid(row=3, column=1, pady=5)
problem_entry = Entry(root, text=sample_problem, width=57, font=55)
problem_entry.grid(row=3, column=2, columnspan=3, pady=5, sticky=W)

alpha_label = Label(root, text="アルファ値 (あるふぁち) :")
alpha_label.grid(row=4, column=3, pady=5)
alpha_entry = Entry(root, text=alpha_val, width=20, font=55)
alpha_entry.grid(row=4, column=4, pady=5, sticky=W)

omega_label = Label(root, text="オメガ値 (おめがち) :")
omega_label.grid(row=4, column=1, pady=5)
omega_entry = Entry(root, text=omega_val, width=20, font=55)
omega_entry.grid(row=4, column=2, pady=5, sticky=W)

# Create and position the "Draw Plot" button
btn = Button(root, text="描く (かく) ...", command=show_plot, width=30)
btn.grid(row=5, column=0, columnspan=5, padx=20, pady=5)

# Create and position the label with hyperlink
owner = Label(root, text="Mr.Hidden - テレグラム: @Darker1063", fg="blue", cursor="hand2")
owner.grid(row=7, column=0, columnspan=5, pady=5)
owner.bind("<Button-1>", open_url)  # Bind the label click event to open the URL

owner2 = Label(root, text="J4RVIS - me.amsl.ir - テレグラム: @J4RVIS", fg="blue", cursor="hand2")
